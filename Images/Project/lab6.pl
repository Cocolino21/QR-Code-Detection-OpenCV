...
% false.
 ...
 replace(_,_,[],[]).
 replace(X, Y, [X|T], R):-  atomic(X), !, replace(X,Y,T,R1), R = [Y | R1].
 replace(X, Y, [H|T], R):-  atomic(H), !, replace(X,Y,T,R1), R = [H | R1].
...
% R = [[3],[1],[1,[3],1],2].
 len_con_depth([], Acc,[Acc]).
 len_con_depth([H|T], Acc, R) :- atomic(H), !, NewAcc is Acc + 1, len_con_depth(T, NewAcc, R).  
 len_con_depth([H|T], Acc, R) :- len_con_depth(H,0,R1), len_con_depth(T,0,R2), append([Acc|R1],R2,R).

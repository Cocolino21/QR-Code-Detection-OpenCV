...
% false
 ...
 del_min(L, R):- min2(L,Min), delete_all(Min,L,R),!.


 del_max(L, R):- max2(L,Max), delete_all(Max,L,R),!.
...
del_min1([], Mp, Mp, []).
del_min1([H|T], Mp, M, R) :- H<Mp, !, del_min1(T, H, M,R), H=<M.
del_min1([H|T], Mp, M, [H|R]) :- H<Mp, !, del_min1(T, H, M,R), H>M.
del_min1([H|T], Mp, M, R) :- del_min1(T, Mp, M,R),H=<M.
del_min1([H|T], Mp, M, [H|R]) :- del_min1(T, Mp, M,R),H>M.

del_min1([H|T], M, R) :- del_min1(T, H, M, R).
...
% false
reverse_k([],_,[]).
reverse_k([H|T],K,R):-K==0,reverse1(T,R1),append1(R1,[H],R).
